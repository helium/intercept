...
-spec unstick() -> ok.
unstick() ->
    [ok = code:unstick_dir(code:lib_dir(L, ebin))
     || L <- [kernel, stdlib, compiler]],
    ok.


%%--------------------------------------------------------------------
%% @doc
%% Compile the abstract code `AC' and load it into the code server.
%% @end
%%--------------------------------------------------------------------
...
compile_and_load(Module, AC, OutDir) ->
    ...
    _ = is_list(OutDir) andalso file:write_file(filename:join(OutDir, ModStr ++ ".beam"), Bin),
...
%%--------------------------------------------------------------------
%% @doc
%% Make the abstract code for the proxy module.  The proxy module
...
%%--------------------------------------------------------------------
%% @doc
%% Apply intercepts to the abstract code `AC' based on `Mapping'.
%% @end
%%--------------------------------------------------------------------
...
%%--------------------------------------------------------------------
%% @doc
%% Return a form modifier function that uses `Mapping' to
...
%%--------------------------------------------------------------------
%% @doc
%% Modify the abstract code `AC' to forward all function calls to
...
%%--------------------------------------------------------------------
%% @doc
%% Return a function which folds over the abstract code of a
...
%%--------------------------------------------------------------------
%% @doc
%% Modify all function definitions in the abstract code `AC' to
...
%%--------------------------------------------------------------------
%% @doc
%% Modify the function `Form' to forward to `Module:Fun'.
%% @end
%%--------------------------------------------------------------------
...
%%--------------------------------------------------------------------
%% @doc
%% Convert generate_fun_wrapper freevars to abstract code
%% @end
%%--------------------------------------------------------------------
generate_freevars([], L) ->
    {nil, L};
...
%%--------------------------------------------------------------------
%% @doc
%% Convert one freevar to abstract code
%%
...
%%--------------------------------------------------------------------
%% @doc
%% Add the `export_all' compile directive to the abstract code `AC'.
%% @end
%%--------------------------------------------------------------------
export_all(AC) ->
    [A, B|Rest] = AC,
...
%%--------------------------------------------------------------------
%% @doc
%% Apply the form modify `F' to all forms in `AC' that are
...
%%--------------------------------------------------------------------
%% @doc
%% Get the abstract code for `Module'.  This function assumes
